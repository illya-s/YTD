try:
        ...
        def run(self):
            try:
                if self.ot == 0:
                    ...
                    stream = youtube.streams.filter(only_audio=bool(self.mp), file_extension=f"{"mp4" if self.mp == 0 else "mp3"}").first()
                    chars_to_remove = ["|", "<", ">", ":", "\"", "\\", "/", "?", "*"]
                    title = youtube.title
                    for char in chars_to_remove:
                        title = title.replace(char, "")
                    stream.download(output_path=self.path, filename=f'{title}.{"mp4" if self.mp == 0 else "wav"}')
                    self.l = 100
                    self.signals.progress.emit(self.l, f'1/1 {title}.{"mp4" if self.mp == 0 else "wav"}')
                    ...
                    for i in link.video_urls:
                        ...
                        self.signals.progress.emit(int(self.l), f'{p}/{len(link)} {title}.{"mp4" if self.mp == 0 else "wav"}')
                    self.signals.finished.emit()
                else:
    ...
    class MainWindow(QMainWindow):
        ...
        def set_progress(self, progress, worck):
            self.ui.progressBar.setValue(progress)
            item = QtGui.QStandardItem(worck)
            self.model.appendRow(item)
            self.ui.listView.setModel(self.model)

        def thread_complete(self):
            self.ui.progressBar.setValue(100)
            item = QtGui.QStandardItem('completed')
            self.model.appendRow(item)
            self.ui.listView.setModel(self.model)
